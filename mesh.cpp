...
void Grid::read_mesh(const char *mesh_file_name)
{
    int nbdry_edge;
    BdryEdge *bdry_edges;
...
#define ERROR {n = __LINE__; goto error;}
    ...
    while (true) { // end at line 217
		...
		if (!strcasecmp(token, "$Nodes")) {
			...
			int nedge_read = 0, ntria_read = 0, nquad_read = 0;
			int elem_type = 0;
			...
			for (i = 0; i < n; i++) {
				...
				assert(elem_type == 1 || elem_type == 2 || elem_type == 3);
            ...
            fprintf(stderr, "number of quadrilaterals: %d\n", nquad_read);
			nelem = ntria_read + nquad_read;
